...
$(document).ready(function () {
    initializeMultiselectionInitial();
...
function initializeMultiselectionInitial(idsSelected) {
    var array = [];
    selectedValues = [];
    if (idsSelected) {
    ...
    $('#poInitialStatuses').multiselect(
        {
            onChange: function (option, checked, select) {
                ...
                $('#poInitialStatusesHidenId').val(selectedValues.toString());
    ...
    if (idsSelected) {
        ...
        $('#poInitialStatuses').multiselect('select', array);
    }
}
...
function deletedExcludeValues(status1, status2){
    ...
    if(index1>-1){
        $('#poInitialStatuses').multiselect('deselect', [status1.toString()]);
        selectedValues.splice(index1,1);
    }
    if(index2>-1){
        $('#poInitialStatuses').multiselect('deselect', [status2.toString()]);
