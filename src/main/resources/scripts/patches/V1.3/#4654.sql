...
INSERT INTO ATTACHMENT_COMMENT (COMMENT_ID,FILE_NAME,MIME_TYPE,FILE_ATTACHED,STATUS_ID,LAST_UPDATE) (
  SELECT ID,F_NAME,MIME_TYPE,FILE_ATTACHED,STATUS_ID,LAST_UPDATE FROM COMMENTS
    WHERE STATUS_ID=1 and F_NAME IS NOT NULL AND MIME_TYPE IS NOT NULL
);

--ALTER TABLE ATTACHMENT_COMMENT ALTER COLUMN FILE_NAME SET NOT NULL;
--ALTER TABLE ATTACHMENT_COMMENT ALTER COLUMN MIME_TYPE SET NOT NULL;

ALTER TABLE COMMENTS DROP COLUMN F_NAME;
ALTER TABLE COMMENTS DROP COLUMN MIME_TYPE;
ALTER TABLE COMMENTS DROP COLUMN FILE_ATTACHED;
...
SELECT ROW_NUMBER() OVER () AS ID, PO.ID AS PO_ID,
	PO.PROJECT, PO.PO, PO.VARIATION, PO.PO_TITLE, S.SUPPLIER,PO.RESPONSIBLE_EXPEDITING, PO.PO_INCO_TERM
