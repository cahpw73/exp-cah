...
DROP TABLE IF EXISTS MODULE_GRANTED_ACCESS CASCADE;
...
DROP SEQUENCE IF EXISTS VERIFICATION_TOKEN_ID_SEQ CASCADE;
DROP SEQUENCE IF EXISTS TRANSIT_DELIVERY_POINT_SEQ CASCADE;
DROP SEQUENCE IF EXISTS TRANSIT_DELIVERY_POINT_SEQ CASCADE;
DROP SEQUENCE IF EXISTS BRAND_ID_SEQ CASCADE;
DROP SEQUENCE IF EXISTS CATEGORY_ID_SEQ CASCADE;
DROP SEQUENCE IF EXISTS MODULE_GRANTED_ACCESS_ID_SEQ CASCADE;
...
CREATE SEQUENCE MODULE_GRANTED_ACCESS_ID_SEQ INCREMENT 1 START 100;
CREATE SEQUENCE USER_ROLE_ID_SEQ INCREMENT 1 START 100;
...
CREATE TABLE MODULE_ENTITY (
	ID integer NOT NULL,
	NAME varchar(50) NOT NULL,
	MODULE_SYSTEM integer NOT NULL
);

CREATE TABLE MODULE_GRANTED_ACCESS (
	ID integer NOT NULL,
...
ALTER TABLE MODULE_GRANTED_ACCESS ADD CONSTRAINT PK_MODULE_GRANTED_ACCESS
	PRIMARY KEY (ID);
...
ALTER TABLE MODULE_GRANTED_ACCESS ADD CONSTRAINT FK_USER_ID
...
SELECT ROW_NUMBER() OVER () AS ID, PO.ID AS PO_ID,
	PO.PROJECT, PO.PO, PO.VARIATION, PO.PO_TITLE, S.COMPANY,PO.RESPONSIBLE_EXPEDITING, PO.PO_INCO_TERM, PO.PO_DELIVERY_DATE,PO.REQUIRED_DATE,
	PURCHASE_ORDER_STATUS
   FROM PURCHASE_ORDER  PO INNER JOIN p_order por on por.id=po_id
   LEFT JOIN SUPPLIER_PROC S ON POR.SUPPLIER_ID=S.ID
   WHERE PO.STATUS_ID<>3 and por.po_status=0
